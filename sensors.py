...
while True:
    ...
    mqttc.publish("moxy/current/1", getReadingFromBytes(response, 1))
    mqttc.publish("moxy/current/2", getReadingFromBytes(response, 2))
    mqttc.publish("moxy/current/3", getReadingFromBytes(response, 3))
    mqttc.publish("moxy/current/4", getReadingFromBytes(response, 4))
    time.sleep(.4)
